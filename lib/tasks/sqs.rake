...
namespace :sqs do
  ...
  task :create, [:env] do |t, args|
    ...
    ['default', 'image_callback', 'audio_callback'].each do |queue|
  ...
  def create_queue(queue, dlq_arn, options={})
    sqs = Aws::SQS::Client.new(endpoint: 'http://192.168.99.100:4576', verify_checksums: false)
  ...
  def create_dlq(queue, options)
    ...
    attrs = sqs.get_queue_attributes(queue_url: dlq.queue_url.sub('localhost', '192.168.99.100'), attribute_names: ['QueueArn']) rescue nil
